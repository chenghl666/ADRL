...
def calibrate_patient_by_admission(patient_admission: dict, admission_codes: dict):
    ...
    for pid, admissions in patient_admission.items():
        for admission in admissions:
            if admission['adm_id'] not in admission_codes:
                break
        else:
            continue
        del_pids.append(pid)
    for pid in del_pids:
        ...
        for admission in admissions:
            if admission['adm_id'] in admission_codes:
                del admission_codes[admission['adm_id']]
            else:
                print('\tpatient %d have an admission %d without diagnosis' % (pid, admission['adm_id']))
        del patient_admission[pid]



def calibrate_patient_by_admission2(patient_admission: dict, admission_codes: dict):
    print('calibrating patients by admission ...')
    del_pids = []
    for pid, admissions in patient_admission.items():
        for admission in admissions:
